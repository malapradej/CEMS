#!~/venv/bin/python
'''
LUT entry for libRadtran atmospheric path reflectance, double transmittance and
spherical albedo. This notebook is for the final LUT which will be used to
create the fast forward model used in surface reflectance retrievals. A
random selection of distributed atmospheric parameters will be used as inputs
to the Emulator.
...
lut_<sol_zen>_<sat_zen>_<rel_azi>_<alt>_<aot>_<wvc>_<press>.p
...
inp_template = 'data_files_path /home/users/malapradej/local/share/libRadtran/data/\
...
def LUT_select(sz, vz, ra, al, wl_min, wl_max, at, ao, wv, pr):
    '''A function that creates a look-up table entry based on the parameters
    provided.
    '''
    descr = ['lam', 'atm_path', 'dbl_trans', 'spher_alb', 'sol_zen', 'sat_zen',\
        'rel_azi', 'alt', 'AOT', 'WVC', 'press']
    que1 = Queue()
    que2 = Queue()
    que3 = Queue()
    ...
    p3.join()
    vals = [lam, apr, dtt, sal, sz, vz, ra, al, ao, wv, pr]
...
exten = '_%s_%s_%s_%s_%s_%s_%s' % (sys.argv[1], sys.argv[2], sys.argv[3], \
