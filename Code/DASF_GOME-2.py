...
path_coda = '/opt/anaconda/share/coda/definitions'
if not(os.path.isdir(path_coda)):
    text = 'The path %s does not exist.' % path_coda
    raise IOError(text)
os.putenv('CODA_DEFINITION', path_coda)
# sets the LIBRADTRAN data files path. This includes all files needed for 
# atmospheres etc.
path_libradtran = '/opt/anaconda/share/libRadtran/data/'
if not(os.path.isdir(path_libradtran)):
    text = 'The path %s does not exist.' % path_libradtran
    raise IOError(text)
import coda
#import beatl2
import numpy as np
...
for cur_scan in np.arange(n_mdr):
    #pdb.set_trace()
    print 'current scan line is %d out of %d scans.' % (cur_scan, n_mdr)
    ...
    for cur_pnt in forw_scans:  
        ...
        # calculate directional hemispherical reflectance at TOA. 
        # this can be replaced by ground level reflectance taking in
        # consideration atmospheric RT....
        irr_arr = np.array(irr)
        alb_toa_arr = rad_arr * np.pi / (irr_arr * np.cos(sol_zen*np.pi/180.) \
        ...
        alb_sur_arr = (alb_toa_arr - ref_atm_arr) / (t1_arr * t2_arr - sph_alb_arr*\
            ref_atm_arr + sph_alb_arr*alb_toa_arr)
        #'''
        '''
        # temporary plot of albedo
        plt.plot(rad_lam, alb_sur_arr, 'r-', label='surface albedo')
        plt.plot(rad_lam, t1_arr*t2_arr, 'b--', label='transmittance')
        plt.plot(rad_lam, alb_toa_arr, 'g-', label='TOA albedo')
        plt.plot(rad_lam, ref_atm_arr, 'b-', label='atmospheric albedo')
...
pf.close()
print 'Saved DASF data to %s file.' % d_fn
