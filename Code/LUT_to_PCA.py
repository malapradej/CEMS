...
rootdir = '/work/scratch/malapradej/DASF/Data/LUT3'
print 'starting pca in root folder %s.' % rootdir
j = 0
for dirname, subdirs, files in os.walk(rootdir):
    # leave out the log file directory
    if 'log' in subdirs:
        subdirs.remove('log')
    if len(subdirs) > 0:
        continue
    j += 1
    print 'pca in folder nr %d - %s.' % (j, dirname)
